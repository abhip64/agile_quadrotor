...
upenn_controller::upenn_controller(const ros::NodeHandle& nh): nh_(nh){
  ...
  nh_.param<double>("/trajectory_generator/max_acc", max_fb_acc_,5.0);

  //Thrust scaling factor
  nh_.param<double>("/trajectory_generator/thrust_scaling_factor", norm_thrust_const_, 0.040);

  //Position control gains along each axis
  nh_.param<double>("/trajectory_generator/Kp_x", Kpos_x_, 15.0);
  nh_.param<double>("/trajectory_generator/Kp_y", Kpos_y_, 15.0);
  nh_.param<double>("/trajectory_generator/Kp_z", Kpos_z_, 15.0);

  //Velocity control gains along each axis
  nh_.param<double>("/trajectory_generator/Kv_x", Kvel_x_, 10.0);
  nh_.param<double>("/trajectory_generator/Kv_y", Kvel_y_, 10.0);
  nh_.param<double>("/trajectory_generator/Kv_z", Kvel_z_, 10.0);

  //Rotation Matrix error gains
  nh_.param<double>("/trajectory_generator/Kr"  , Kr     , 14.0);
  //Angular velocity error gains
  nh_.param<double>("/trajectory_generator/Kw"  , Kw     ,  0.3);
